...
public class PearlNecklaceItem extends Item {
    ...
    public void appendHoverText(ItemStack itemStack, TooltipContext context, List<Component> components, TooltipFlag tooltipFlag) {
        ...
        components.add(Component.translatable("pearl_necklace.swim_speed", percentage(level, CACCommonConfig.HANDLER.instance().necklace.swimSpeed)).withStyle(ChatFormatting.GRAY));
        components.add(Component.translatable("pearl_necklace.drowned_range", percentage(level, CACCommonConfig.HANDLER.instance().necklace.drownedRangeDebuff)).withStyle(ChatFormatting.GRAY));
        if (level > 1) {
            components.add(Component.translatable("pearl_necklace.guardian_range", percentage(level, CACCommonConfig.HANDLER.instance().necklace.guardianRangeDebuff)).withStyle(ChatFormatting.GRAY));
        }
    }
