...
public class CACSpawnConfig {
    ...
    public record SpawnEntry(String biomeSpec, int weight, int min, int max) {
        ...
        public static SpawnEntry parse(String raw) {
            ...
            ResourceLocation.parse(biomeSpec.startsWith("#") ? biomeSpec.substring(1) : biomeSpec);
    ...
    public static SpawnEntry cTag(String tagPath, int weight, int min, int max) {
        return new SpawnEntry("#c:" + tagPath, weight, min, max);
    }
