...
public class JumpingSpiderEntity extends TamableAnimal implements IAnimatable {
    ...
    static class JumpingSpiderMoveControl extends MoveControl {
        ...
        public void tick() {
            if (this.hasWanted() && this.spider.isOnGround() && this.spider.getRandom().nextFloat() <= 0.1F) {
                this.spider.setDeltaMovement(this.spider.getDeltaMovement().add(0.0D, 0.4D, 0.0D));
            }
            super.tick();
        }
    }
